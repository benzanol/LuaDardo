...
class Lexer {
          ...
          switch (chunk.current) {
            ...
            if (!CharSequence.isDigit(chunk.current)) {
              ...
              for (int i = 0; i < 3 && CharSequence.isDigit(chunk.current); i++) {
                final asInt = int.parse(chunk.current);
                c = 10 * c + asInt;
                chunk.next(1);
              }
              _save_c(c);
  ...
  String readNumeral() {
    RegExp expo = RegExp("[Ee]");
    String first = chunk.current;
    _save_and_next();
    if (first == '0' && chunk.startsWith(RegExp("[xX]")))  /* hexadecimal? */
    expo = RegExp("[Pp]");

    for (;;) {
      if (chunk.startsWith(expo)) {  /* exponent part? */
        _save_and_next();
        if (chunk.startsWith(RegExp("[-+]"))) _save_and_next();  /* optional exponent sign */
